...
class Context{
    ...
    getLayoutTypeByScreenDimension(layoutSuported){
        let screenSize = this.getScreenSize();
        let width = screenSize.width;
        if(this.getScreenOrientation() === 'VERTICAL')
            width = screenSize.height;
        if(Array.isArray(layoutSuported)){
            if(width <30 && layoutSuported.includes(Context.LAYOUT_COMPACT)){ // COMPACTO (CELULARES)
                return Context.LAYOUT_COMPACT;
            }else if(width < 40 && layoutSuported.includes(Context.LAYOUT_MEDIUM)){ // MEDIANO (TABLETS)
                return Context.LAYOUT_MEDIUM;
            }else if(width < 60 && layoutSuported.includes(Context.LAYOUT_EXPANDED)){ // EXPANDEND (LAPTOPS O PC)
                return Context.LAYOUT_EXPANDED;
            }else{ // BIG (TELEVISORES O MONITORES GRANDES)
                return Context.LAYOUT_BIG;
            }
        }else{
...
class LinkButton extends TextView{
    constructor(context,model){
        super(context,model);
        this.margin.left = this.margin.top = this.margin.right = this.margin.bottom = 4;
    }
    ...
    async onMeasure(maxWidth, maxHeight){
        ...
        while(index < visibles.length){
            ...
            for(let j=1; j <=this.colums && index < visibles.length; j++) {
                ...
                await view.onMeasure(maxAnchoView , maxHeight);
