...
var LayoutInflater = {
     ...
     */
    parseSync: async function (context, firstElement) {
        var view = null;
        try {
        ...
        await view.parseSync(firstElement);
        return view;
    },
    // createView: function (context, domXmlElement) {
    //     return this.parse(context, domXmlElement);
    // },
    inflateFromURLSync: async function (context, urlXmlLayout,subContextPath) {
        let viewXmlRoot = await loadLayoutSync(urlXmlLayout);
        var view = await this.parseSync(context, xmlhttp.responseXML.documentElement);
        return view;
    }
...
View = Class.extend({
    parentView: null,
    buided: false, // Indica si el objeto fue agregado a una vista creando los elementos DOM para HTML
    margin: null,
    padding: null,
    context: null,
